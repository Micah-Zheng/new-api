...
func defaultImagePerSizePrice(modelName, sizeTier string) float64 {
	...
	// gpt-image-2: OpenAI official per-image pricing (high quality, 1024×1024
	// equivalent for each tier). Admins should override these to match their
	// upstream cost.
	if strings.HasPrefix(modelName, "gpt-image-2") {
		switch sizeTier {
		case ImageSizeTier1K:
			return 0.011 // low-quality equivalent
		case ImageSizeTier2K:
			return 0.042 // medium-quality equivalent
		case ImageSizeTier4K:
			return 0.167 // high-quality equivalent
		}
	}
