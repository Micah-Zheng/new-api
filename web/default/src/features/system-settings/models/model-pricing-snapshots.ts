...
export const getModeVariant = (
  ...
  if (mode === 'per-resolution') return 'orange'
  ...
  if (row.billingMode === 'per-request') {
    if (row.perRequestSubMode === 'per-resolution') {
      ...
      return parts.length > 0
        ? parts.join(' · ')
        : t('Per-resolution image pricing')
    }
...
export const buildModelSnapshots = ({
  ...
  const imageModelMap = safeJsonParse<Record<string, ImageModelConfig>>(
    imageModelSetting,
    {
      fallback: {},
      context: 'image model settings',
    }
  )
  ...
  return Array.from(modelNames).map((name) => {
    ...
    const imageModelConfig = imageModelMap[name]
    const isPerResolution = imageModelConfig?.billing_mode === 'per_size'
    ...
    if (modeForModel === 'tiered_expr') {
      ...
      return {
        ...
        price1k:
          imageModelConfig?.price_1k != null
            ? String(imageModelConfig.price_1k)
            : '',
        price2k:
          imageModelConfig?.price_2k != null
            ? String(imageModelConfig.price_2k)
            : '',
        price4k:
          imageModelConfig?.price_4k != null
            ? String(imageModelConfig.price_4k)
            : '',
        hasConflict: false,
      }
    }
