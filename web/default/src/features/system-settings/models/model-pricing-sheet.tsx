    ...
    if (editData) {
      ...
      setPricingMode(
        editData.billingMode === 'tiered_expr'
          ? 'tiered_expr'
          : editData.perRequestSubMode === 'per-resolution' || editData.price
            ? 'per-request'
            : 'per-token'
      )
  ...
  const buildSubmitData = useCallback(
    (values: ModelPricingFormValues) => {
      ...
      if (pricingMode === 'per-request') {
        data.perRequestSubMode = perRequestSubMode
    ...
    [billingExpr, perRequestSubMode, price1k, price2k, price4k, pricingMode, requestRuleExpr]
  )
                    ...
                    <FieldGroup className='gap-5'>
                      <div className='grid grid-cols-2 gap-2'>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setPerRequestSubMode('per-resolution')
                          }
                        >
                          {t('Per-resolution')}
                        </Button>
                      ...
                      {perRequestSubMode === 'per-resolution' && (
                        <Field>
                          <FieldLabel>{t('Per-resolution prices')}</FieldLabel>
                          <div className='grid gap-3 sm:grid-cols-3'>
                            {[
                              ...
                              <InputGroup key={item.label}>
                                <InputGroupAddon>{item.label}</InputGroupAddon>
                                <InputGroupInput
                                  inputMode='decimal'
                          ...
                          <FieldDescription>
                            {t(
                              'Flat USD price per generated image. Leave empty to use the built-in default price.'
                            )}
                          </FieldDescription>
                        </Field>
                      )}
