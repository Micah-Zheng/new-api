...
import { ExternalLink, Loader2, Palette, Sparkles, TriangleAlert } from 'lucide-react'
...
// Must end with trailing slash to avoid Nginx 301 redirect which escapes the iframe.
...
const WELCOME_SHOWN_KEY = 'image-playground-welcome-shown'
...
function ImagePlayground() {
  ...
  const keyWasNewlyCreated = useRef(false)
  ...
  // Show welcome dialog the first time a key is auto-created for this user.
  useEffect(() => {
    if (!isSuccess || !apiKey) return
    const alreadyShown = localStorage.getItem(WELCOME_SHOWN_KEY)
    if (!alreadyShown) {
      keyWasNewlyCreated.current = true
      setShowWelcome(true)
    }
  }, [isSuccess, apiKey])
      ...
      <AlertDialog open={showWelcome} onOpenChange={(open) => { if (!open) handleWelcomeDismiss() }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <span className='block space-y-2 text-left text-sm'>
                <span className='block'>{t('We\'ve automatically created a dedicated API key "{{name}}" for you.', { name: DEDICATED_KEY_NAME })}</span>
                <span className='block'>{t('This key is linked to the image generation channel. You will only be charged when you actually generate an image — browsing and configuring the playground is free.')}</span>
                <span className='block'>{t('You can view or manage this key anytime in your API Keys page.')}</span>
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Main className='p-4'>
        <div className='bg-background relative min-h-0 flex-1 overflow-hidden rounded-xl border'>
          ...
          <iframe
            ...
            // allow-same-origin is required for the embedded app's localStorage (zustand persist).
            // allow-top-navigation is intentionally omitted to prevent the iframe from
            // escaping to a full-page view when the user refreshes inside it.
